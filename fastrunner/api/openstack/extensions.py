...
import fastrunner.api.openstack
...
class ExtensionManager(object):
    ...
    See fastrunner/tests/api/openstack/compute/extensions/foxinsocks.py or an
...
class ControllerExtension(object):
    """Extend core controllers of fastrunner OpenStack API.

    Provide a way to extend existing fastrunner OpenStack API core
    controllers.
    """
...
class ResourceExtension(object):
    """Add top level resources to the OpenStack API in fastrunner."""
...
def core_authorizer(api_name, extension_name):
    def authorize(context, target=None, action=None):
        ...
        fastrunner.policy.enforce(context, act, target)
    return authorize
...
def check_compute_policy(context, action, target, scope='compute'):
    ...
    fastrunner.policy.enforce(context, _action, target)
        ...
        def wrapped(*args, **kwargs):
            try:
                ...
                msg = _('Unexpected API Error. Please report this at '
                    'http://bugs.launchpad.net/fastrunner/ and attach the Nova '
                    'API log if possible.\n%s') % type(exc)
